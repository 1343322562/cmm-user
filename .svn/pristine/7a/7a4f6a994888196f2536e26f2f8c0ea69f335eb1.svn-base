...
Page({
    ...
    API.Login.getUserAgreementInfo({
      ...
      success: res => {
        if (res.code == 0) {
          ...
          this.setData({ agreementObj, agreement: agreementObj?false: true})
        }
      }
    })
  },
  ...
  changeShowUserList (type) {
    ...
    this.userListAnimation.height(showUserList ? 0 : ((userList.length * 80)+'rpx')).step()
    this.setData({ showUserList: !showUserList, userListAnimation: this.userListAnimation.export()})
  },
  imgError () {
  ...
  hideLoadingImg (type,fun) {
    setTimeout(()=>{
      if (fun) {
        ...
        this.animation.opacity(0).scale(1.5).step()
